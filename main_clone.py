...
async def run_diagnostic_test():
    ...
    image_path = str(Path.cwd() / "test_vision.jpg")
    if not Path(image_path).exists():
        # If test_vision.jpg not found, find the first .jpg file in cwd
        for file in Path.cwd().iterdir():
            ...
            logger.error("🔱 [Diagnostic Mode]: No JPG image found in working directory. Cannot perform visual reconnaissance.")
            return
